...
const { sendBillingReminder, sendOverdueNotice } = require('../services/notifyService');
...
async function runReminderCheck() {
  ...
  for (const days of reminderDays) {
    ...
    const invoices = db.prepare(`
      SELECT i.*, c.name, c.phone, c.email, c.payment_method, c.notification_channel, c.id as customer_id
      FROM invoices i
        ...
        await sendBillingReminder(
          {
            ...
            notification_channel: inv.notification_channel,
          },
...
async function runOverdueCheck() {
  ...
  const overdue = db.prepare(`
    SELECT i.*, c.name, c.phone, c.email, c.notification_channel, c.id as customer_id
    FROM invoices i
    JOIN customers c ON c.id = i.customer_id
  ...
  for (const inv of overdue) {
    try {
      await sendOverdueNotice(
        { id: inv.customer_id, name: inv.name, phone: inv.phone, email: inv.email, notification_channel: inv.notification_channel },
